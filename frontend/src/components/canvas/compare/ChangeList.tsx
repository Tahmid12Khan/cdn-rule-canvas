...
// 1..N in display order (node changes then edge changes). Each item is
// git-diff coloured and, when clicked, asks the parent to pan/centre the
// canvas on the node / edge midpoint. Modified nodes list their changed fields
// with old→new values.
...
import type { CanvasDiff, FieldChange, NodeDiff } from "@/lib/canvas/diff";
...
interface ChangeListProps {
  diff: CanvasDiff;
  onSelect: (nodeId: string | null, point: { x: number; y: number }) => void;
}
...
export function ChangeList({ diff, onSelect }: ChangeListProps) {
  ...
  const nodeChanges = diff.nodes.filter((n) => n.status !== "unchanged");
  const edgeChanges = diff.edges.filter((e) => e.status !== "unchanged");
  const posById = new Map(diff.nodes.map((n) => [n.id, n.node.position]));
    ...
    >
      ...
      <ul className="space-y-1.5">
        {nodeChanges.map((nd: NodeDiff) => {
          ...
          return (
            <li key={nd.id}>
              <button
                type="button"
                onClick={() => onSelect(nd.id, nodeCenter(nd.node))}
        ...
        {edgeChanges.map((ed) => {
          counter += 1;
          const n = counter;
          const srcNode = diff.nodes.find((x) => x.id === ed.source)?.node;
          const tgtNode = diff.nodes.find((x) => x.id === ed.target)?.node;
