...
// Client component: controlled Radix Dialog confirming deletion of a test
// preset. Mirrors SiteDeleteDialog. The list client owns the open state + which
// preset is targeted.
...
import { deleteTestPreset, type TestPresetRead } from "@/lib/api/test-presets";
import { toUserError, type UserError } from "@/lib/errors/userError";

interface TestPresetDeleteDialogProps {
  preset: TestPresetRead | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function TestPresetDeleteDialog({
  preset,
  open,
  onOpenChange,
}: TestPresetDeleteDialogProps) {
  ...
  const mutation = useMutation({
    mutationFn: () => {
      ...
      return deleteTestPreset(preset.slug);
    },
    onSuccess: () => {
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl bg-bg-elevated p-6 shadow-xl focus:outline-none">
          ...
          <Dialog.Description className="mt-1 text-sm text-status-prevFg">
            This removes the saved test inputs{" "}
            <code className="font-mono text-fg-muted">{preset?.slug}</code>. This
